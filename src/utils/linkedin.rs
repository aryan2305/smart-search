extern crate percent_encoding;

use percent_encoding::{utf8_percent_encode, AsciiSet, CONTROLS};

// Used as part of the percent_encoding library
const FRAGMENT: &AsciiSet = &CONTROLS.add(b' ').add(b'"')
               .add(b'<').add(b'>').add(b'`');

pub fn construct_linkedin_url(query: &str) -> String {
    if query == "ld" {
        let linkedin_dotcom = "https://linkedin.com";
        
        linkedin_dotcom.to_string()
        
        // Check if it looks like a linkedin profile
        } else if &query[..4] == "ld @" {
            construct_linkedin_profile_url(&query[4..])
        } else {
            // Assume the other match is "ld sometext"
            // and search on linkedin
            construct_linkedin_search_url(&query[3..])
    }
}

pub fn construct_linkedin_profile_url(profile: &str) -> String {
    format!("https://linkedin.com/in/{}", profile)
}

pub fn construct_linkedin_search_url(query: &str) -> String {
    let encoded_query = utf8_percent_encode(query, FRAGMENT)
               .to_string();
    let linkedin_search_url = format!(
    "https://linkedin.com/search/results/all/?keywords={}", encoded_query);
    linkedin_search_url
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_construct_linkedin_url() {
        let fake_query = "ld";
        assert_eq!(construct_linkedin_url(fake_query), 
        "https://linkedin.com");
    }

    #[test]
    fn test_construct_linkedin_url_query() {
        let fake_query = "ld hello world";
        assert_eq!(construct_linkedin_url(fake_query),     
        "https://linkedin.com/search/results/all/?keywords=hello%20world");
    }

    #[test]
    fn test_construct_linkedin_url_profile() {
        let fake_query = "ld @aryan2305";
        assert_eq!(construct_linkedin_url(fake_query), 
        "https://linkedin.com/in/aryan2305");
    }

    #[test]
    fn test_construct_linkedin_profile_url() {
        let fake_profile = "jsjoeio";
        assert_eq!(
            construct_linkedin_profile_url(fake_profile),
            "https://linkedin.com/in/jsjoeio"
        );
    }

    #[test]
    fn test_construct_linkedin_search_url() {
        let fake_query = "hello world";
        assert_eq!(
            construct_linkedin_search_url(fake_query),
            "https://linkedin.com/search/results/all/?keywords=hello%20world"
        );
     }
}